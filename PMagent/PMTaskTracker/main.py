...
from crew.crew import CrewaiProjectManagerTaskTrackingagent
...
def run():
    ...
    while True:
        ...
        response = CrewaiProjectManagerTaskTrackingagent().crew().kickoff(inputs=inputs)

        # Add chatbot response to memory
        #memory.add(f"Assistant: {response}", user_id="assistant")
        print(f"PM Agent: {response}")
